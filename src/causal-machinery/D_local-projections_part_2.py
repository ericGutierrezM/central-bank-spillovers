...
# Dictionary to collect all tables for the final terminal printout
...
for llm in LLMS:
    print(f"\n{'='*50}")
    print(f"=== RUNNING LOCAL PROJECTIONS FOR: {llm.upper()} ===")
    print(f"{'='*50}")
    ...
    for market_name, params in markets.items():
        ...
        for h in range(max_h + 1):
            ...
            for bank in BANKS:
                ...
                irf_results[market_name][bank]['ci_lower'].append(coef - 1.96 * se)
                irf_results[market_name][bank]['ci_upper'].append(coef + 1.96 * se)
    ...
    print(f"  -> Saved CSV Matrices for h={horizons_to_report}")

# ==========================================
# 4. THE COPY-PASTE ZONE
# ==========================================
print("\n" * 3)
print("*" * 80)
